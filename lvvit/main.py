...
def main():
    ...
    if args.num_classes is None:
        ...
        args.num_classes = model.num_classes  # hisFIXME handle model default vs config num_classes more elegantly
    ...
    try:
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            train_metrics = train_one_epoch(
                ...
                model_ema=model_ema, mixup_fn=mixup_fn, optimizers=optimizers,
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            ...
            if args.lottery and model_pretrained is not None:
                ...
                mask1 = torch.zeros(batch_size_here, left_tokens, 1, requires_grad=False).to(input.device,
                                                                                             non_blocking=True)
                for b in range(batch_size_here):
                    ...
                    mask_tt = mask_tt.to(input.device, non_blocking=True)
                    mask1[b, :, :] = mask_tt
                ...
                mask2 = torch.zeros(batch_size_here, left_tokens, 1, requires_grad=False).to(input.device,
                                                                                             non_blocking=True)
                for b in range(batch_size_here):
                    mask_tt = np.random.choice(left_tokens_pre, left_tokens, replace=False)
                    mask_tt = np.array([mask_tt])
                ...
                left_tokens_pre = left_tokens
                left_tokens = math.ceil(keep_rate * left_tokens)
                mask3 = torch.zeros(batch_size_here, left_tokens, 1, requires_grad=False).to(input.device,
                                                                                             non_blocking=True)
                for b in range(batch_size_here):
                    mask_tt = np.random.choice(left_tokens_pre, left_tokens, replace=False)
                    mask_tt = np.array([mask_tt])
