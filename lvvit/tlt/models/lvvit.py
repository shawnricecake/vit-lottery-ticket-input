...
class LV_ViT(nn.Module):
    ...
    def forward_embeddings(self,x):
        ...
        return x
    def forward_tokens(self, x, keep_rate=None):
        B = x.shape[0]
    ...
    def forward(self, x, keep_rate=None):
        ...
        x = x.flatten(2).transpose(1, 2)
        x = self.forward_tokens(x, keep_rate=keep_rate)
        x_cls = self.head(x[:,0])
...
@register_model
def lvvit_small_sparse(pretrained=False, base_keep_rate=0.85, drop_loc=(4, 8, 12), **kwargs):
    keep_rate = [1] * 16
    for loc in drop_loc:
        keep_rate[loc] = base_keep_rate
    model = LV_ViT(patch_size=16, embed_dim=384, depth=16, num_heads=6, mlp_ratio=3.,
