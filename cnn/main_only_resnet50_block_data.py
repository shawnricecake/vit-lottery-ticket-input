...
class TeacherStudentDistillNet(nn.Module):
    ...
    def compute_ts_distill_loss(self):
        feature_loss = 0.0
        for tf, sf, proj_conv in zip(self.teacher_stage_features_list, self.student_stage_features_list,
                                     self.proj_conv_list):
...
def config_dist_env_and_opt(opt):
    ...
    if opt.dist_mode == 'cpu':
        ...
        opt.rank = mpi_rank
        # opt.AutoGPU = global_utils.AutoGPU()
        opt.gpu = int(mpi_rank)  # to replace the auto gpu, we here straightly use the rank of mpi to call gpu
        torch.cuda.set_device(opt.gpu)
        ...
        opt.rank = hvd.rank()
    elif opt.dist_mode == "distribute":  # xuan
        pass
    else:
...
def train_one_epoch(train_loader, model, criterion, optimizer, epoch, opt, num_train_samples,
                    no_acc_eval=False,
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # block data
        #########################################################################################################
        batch_size_here = input.shape[0]
        if opt.lottery and lottery_pretrained_model is not None:
            outputs = lottery_pretrained_model(input, keep_rate=opt.base_keep_rate)
            all_index_record = []
            for e in lottery_pretrained_model.blocks:
            ...
            input = global_utils.go_back_original_input_with_zero(index=total_original_index,
        ...
        if opt.teacher_arch is not None:
        ...
        # xuan: remove eval in iteration of one epoch
        # if not no_acc_eval:
        ...
        if i % opt.print_freq == 0 and opt.rank == 0:
            if opt.teacher_arch is not None:
                logging.info('Train epoch={}, i={}, loss={:.4f},'
                             'logit_loss={:.4f}, ts_feature_loss={:.4f}, ts_logit_loss={:.4f},'
                             'lr={:.6f}'.format(
                    epoch, i, float(loss),
...
def validate(val_loader, model, criterion, opt, epoch='N/A'):
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
            ...
            if i % opt.print_freq == 0 and opt.rank == 0:
                logging.info('Eval epoch={}, i={}, loss={:.2f}, acc1={:.2f}%, acc5={:.2f}%'.format(
...
def train_all_epochs(opt, model, optimizer, train_sampler, train_loader, criterion, val_loader, num_train_samples=None,
    ...
    for epoch in range(opt.start_epoch, opt.epochs):
        ...
        train_one_epoch_info = train_one_epoch(train_loader, model, criterion, optimizer, epoch, opt, num_train_samples,
                                               ...
                                               lottery_pretrained_model=lottery_pretrained_model)
        ...
        elasped_hour = (time.time() - timer_start) / 60
        remaining_hour = (time.time() - timer_start) / float(epoch - opt.start_epoch + 1) * (opt.epochs - epoch) / 60
        logging.info(
            '--- Epoch={}, Elasped mins={:.1f}, Remaining mins={:.1f}, Training Speed={:4g}, \n'
            ' best_acc1={:.2f}, best_acc1_at_epoch={}, best_acc5={:.2f}, best_acc5_at_epoch={}'.format(
        ...
        if save_params and (opt.rank == 0 or save_all_ranks) and \
            ...
            checkpoint_filename = os.path.join(opt.save_dir, 'latest-model.pth')
        ...
        if save_params and (opt.rank == 0 or save_all_ranks) and \
                (epoch + 1 == 6):
            checkpoint_filename = os.path.join(opt.save_dir, '15epoch-model.pth')
            save_checkpoint(checkpoint_filename, {
                'epoch': epoch,
                'state_dict': model.state_dict(),
                'optimizer': optimizer.state_dict(),
        ...
        if save_params and is_best_acc1 and (opt.rank == 0 or save_all_ranks):
            checkpoint_filename = os.path.join(opt.save_dir, 'best-model.pth')
            save_checkpoint(checkpoint_filename, {
                'epoch': epoch,
                'state_dict': model.state_dict(),
                'top1_acc': acc1,
        ...
        if opt.teacher_arch is not None:
            if save_params and is_best_acc1 and (opt.rank == 0 or save_all_ranks):
                checkpoint_filename = os.path.join(opt.save_dir, 'student-best-model.pth')
                save_checkpoint(checkpoint_filename, {
                    'epoch': epoch,
...
def main(opt, argv):
    ...
    print("Using seed = {}".format(opt.seed))
    ...
    if opt.teacher_arch is not None:
        ...
        tmp_opt.input_image_size = opt.input_image_size
    ...
    if opt.teacher_arch is not None:
        ...
        # xuan:
        arch = opt.arch
        assert arch.find('.py:MasterNet') >= 0
        assert arch.split(':')[1] == 'MasterNet'
        sys.path.append("")
        from nas.models.masternet import MasterNet
        student_model = MasterNet(num_classes=opt.num_classes, argv=sys.argv)
    ...
    lottery_pretrained_model = create_model("deit_small_patch16_shrink_base",
                                            base_keep_rate=opt.base_keep_rate,
                                            pretrained=False,
                                            num_classes=1000,
                                            drop_rate=0.0,
                                            drop_path_rate=0.1,
                                            ...
                                            fuse_token=False,
                                            img_size=(224, 224))
    if opt.lottery.startswith('https'):
        checkpoint = torch.hub.load_state_dict_from_url(opt.lottery, map_location='cpu', check_hash=True)
    else:
        checkpoint = torch.load(opt.lottery, map_location='cpu')
    checkpoint = checkpoint['model']
    missing_keys, unexpected_keys = lottery_pretrained_model.load_state_dict(checkpoint, strict=False)
    print('# missing keys=', missing_keys)
    print('# unexpected keys=', unexpected_keys)
    print('successfully loaded from given checkpoint weights:', opt.lottery)
    ...
    if opt.gpu is not None:
        ...
        lottery_pretrained_model.cuda(opt.gpu)
    ...
    if opt.auto_resume and opt.resume is None:
        latest_pth_fn = os.path.join(opt.save_dir, 'latest-model.pth')
    ...
    # ----- save initial model -----#
    if opt.rank == 0:
        checkpoint_filename = os.path.join(opt.save_dir, 'initial-model.pth')
        save_checkpoint(checkpoint_filename, {
            'state_dict': model.state_dict()
        })
    ...
    if opt.resume:
        ...
        #                                                                 map_location=map_location)
        model= resume_checkpoint(model, optimizer, opt.resume, opt, map_location=map_location)
